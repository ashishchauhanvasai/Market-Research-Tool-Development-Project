...
def main():
    ...
    if st.button("Run Market Research"):
        # Load and display the market research CSV data
        try:
            data = pd.read_csv("market_research.csv")
            st.subheader("Market Research Data")
            st.dataframe(data)
        except FileNotFoundError:
            st.error("The file 'market_research.csv' was not found.")
            return
        ...
        with st.spinner("Conducting market research..."):
            research_output = market_agent.perform_search()
